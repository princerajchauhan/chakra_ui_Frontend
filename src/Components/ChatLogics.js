export const getSender = (loginUser, users) => {
    return users[0].name === loginUser.name ? users[1].name : users[0].name
}

export const getSenderDetail = (loginUser, users) => {
    return users[0].name === loginUser.name ? users[1] : users[0]
}
...
export const isLastMsg = (message, i, userId) => {
    return (
        i === message.length - 1 &&
